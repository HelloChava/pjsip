...
function configure () {
	...
	if [ "$TYPE" == "macos" ]; then
		...
		echo "#undef PJ_ACTIVESOCK_TCP_IPHONE_OS_BG" >> "${PJSIP_CONFIG_PATH}"
		echo "#define PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT 1" >> "${PJSIP_CONFIG_PATH}"
		echo "#define PJ_ACTIVESOCK_TCP_IPHONE_OS_BG 0" >> "${PJSIP_CONFIG_PATH}"
		if [[ ${HAS_VIDEO} ]]; then
